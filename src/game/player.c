...
Player createPlayer() {
  ...
  Rectangle collision_rect = (Rectangle){
      spawn_pos.x, spawn_pos.y, idle_texture.width, idle_texture.height};

  return (Player){.texture = idle_texture,
                  .position = spawn_pos,
...
PlayerAnimations *loadPlayerAnimations() {
  PlayerAnimations *animations =
      (PlayerAnimations *)malloc(sizeof(PlayerAnimations));

  animations->idle_animations[0] =
      LoadTexture("resources/character/idle/1.png");
  animations->idle_animations[1] =
      LoadTexture("resources/character/idle/2.png");
  animations->idle_animations[2] =
      LoadTexture("resources/character/idle/3.png");
  animations->idle_animations[3] =
      LoadTexture("resources/character/idle/4.png");
  animations->idle_animations[4] =
      LoadTexture("resources/character/idle/5.png");

  animations->fall_animations[0] =
      LoadTexture("resources/character/fall/fall.png");

  animations->jump_animations[0] =
      LoadTexture("resources/character/jump/1.png");
  animations->jump_animations[1] =
      LoadTexture("resources/character/jump/2.png");
  animations->jump_animations[2] =
      LoadTexture("resources/character/jump/3.png");
...
void animate(Player *player) {
  ...
  case PLAYER_STATE_IDLE:
    ...
    current_frame =
        player->animations->idle_animations[(int)player->animation_frame];
    break;
  ...
  case PLAYER_STATE_FALLING:
    ...
    current_frame =
        player->animations
            ->fall_animations[(int)player->animation_frame]; // kun 1 animation
    break;
  case PLAYER_STATE_RUNNING:
    checkFrames(player, 6);
    current_frame =
        player->animations->run_animations[(int)player->animation_frame];
    break;

  case PLAYER_STATE_JUMPING:
    checkFrames(player, 3);
    current_frame =
        player->animations->jump_animations[(int)player->animation_frame];
    break;
  default:
    break;
  }
...
void drawPlayer(Player *player, Vector2 offset) {
  ...
  /// WHITE);
  /*
//  DrawTextureV(player->texture, screenPos, WHITE);
  DrawRectangleLines(
    (int)player->collision_rect.x - offset.x,
    (int)player->collision_rect.y - offset.y + y_offset,
