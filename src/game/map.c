...
Map createMap(int level_id, Resources resources) {
  ...
  TileGroup terrain_tile_group = createTileGroup("levels/0/level_0_terrain.csv", resources.terrain_spritesheet.sliced_images, TILE_TERRAIN);
  TileGroup player_setup_group = createTileGroup("levels/0/level_0_player.csv", resources.player_setup_spritesheet.sliced_images, TILE_PLAYER_SETUP);
    ...
    for(int i = 0; i < ROWS; ++i) {
      for(int j = 0; j < COLS; ++j) {
        ...
        if(value != -1) {
          ...
          tile_arr[tiles_size] = createTile(
            texture_ptr,
            position,
            attributes,
            attribute_size
